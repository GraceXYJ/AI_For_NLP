...
while loop_times>0:
    ...
    if error_rate<min_error_rate:
        ...
        print('f(fare)={}*age+{},with error rate:{}'.format(best_k,best_b,min_error_rate))
    loop_times-=1
#plt.scatter(sub_age,sub_fare)
#plt.plot(sub_age,estimated_fares,c='b')
plt.plot(range(len(losses)),losses,c='g')
plt.show()
...
while loop_times>0:
    ...
    k_deita=k_delta_direction*step()
    b_deita=b_delta_direction*step()

    new_k=best_k+k_deita
    new_b=best_b+b_deita
   # best_direction=(k_delta_direction,b_delta_direction)
    ...
    if error_rate<min_error_rate:
        ...
        best_direction = (k_delta_direction, b_delta_direction)
        print(min_error_rate)
        print('loop2={}'.format(10000-loop_times))
        print('f(fare)={}*age+{},with error rate:{}'.format(best_k,best_b,min_error_rate))
    else:
        #best_direction=random.choice(change_directions)
        best_direction=random.choice(list(set(change_directions)-{(k_delta_direction,b_delta_direction)}))#随机选的方向不包含老方向
    loop_times-=1
plt.plot(range(len(losses)),losses,c='g')
plt.show()
plt.plot(range(len(losses[-10:])),losses[-10:],c='g')
plt.show()
